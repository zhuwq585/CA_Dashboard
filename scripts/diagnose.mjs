...
const { ConversationLogReader } = await import(path.join(ROOT, '..', 'dist', 'jsonl', 'conversationLogReader.js'));
const { StatusResolver }        = await import(path.join(ROOT, '..', 'dist', 'resolver', 'statusResolver.js'));
...
const sessions = await Promise.all(files.map(async f =>
	JSON.parse(await fs.readFile(path.join(sessionsDir, f), 'utf-8'))));
...
for (const r of resolved) {
	...
	} catch { children = '<no children>'; }
	console.log('─'.repeat(72));
	console.log(`pid=${s.pid}  cwd=${s.cwd}`);
	console.log(`session.json: status=${s.status ?? '<none>'}  waitingFor=${s.waitingFor ?? '<none>'}  updatedAt=${s.updatedAt ?? '<none>'}`);
